...
def main(args):
    ...
    for name, value in implementations.items():
        ...
        # Get project
        project = gl.projects.get(value["project_id"], lazy=True)
        ...
        try:
            for job in project.jobs.list(all=True):
                if job.ref == ref and job.stage == 'build' and job.status == 'success':
                    artifacts = job.artifact(path='/artifact.zip')
                    ZipFileWithPermissions(io.BytesIO(artifacts)).extractall(path=outpath)
                    logger.info(colored(f'{name}: artifacts pulled successfully.', 'green'))
                    successful += 1
                    break
